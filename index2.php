...
var new_json_dict = {'comp': {'osize': 0.00001794, 'otime': 0.81311926},
...
function dataArrives(){
  var javascript_type_list = new Array('1', '2', '3', 'application:x-javascript', 'application:javascript', 'application:ecmascript',
                            'text:javascript', 'text:ecmascript', 'application:json', 'javascript:text')
  var css_type_list = new Array('4', 'text/css', 'css:text')
	...
	for (var i = 0; i < d.objs.length; i++){
		...
		if (d.objs[i].download){
			...
			if (d.objs[i].download.receiveLast < 0) {
				...
				obj_download.len = 0;
			} if javascript_type_list.indexOf(d.objs[i].download.type) > -1 ||  
                             css_type_list.indexOf(d.objs[i].download.type){
...
<? } ?>
		...
		<span class="right">
			<form action="" method="post" class="toRemove">
				<span>
...
?>
	<div class="container-fluid" id="bg">
		<div id="inval" class="alert alert-danger" role="alert"><b>Invalid URL!</b> Try copy and pasting a url directly from the browser.</div>
		<div id="chooseFile">
			<a href="http://wprof.cs.washington.edu">
	...
	<div class="container-fluid" id="mInfo">
		<div class="page-header"><h1>How it Works</h1></div>
			...
			<p> For more information, go to <a class="purple" href="http://wprof.cs.washington.edu/">http://wprof.cs.washington.edu</a></p>
...
<? } ?>
			<li><a class="purple" href="http://wprofx.cs.stonybrook.edu/">wprofx</a></li>
...
if(isset($_POST['givenURL'])) {
	...
	if(preg_match("/\.mp3$|\.zip$|\.tar$|\.mp4$|\.gif$|\.jpg$|\.png$|\.pdf$|\.exe$/", $link) || !filter_var($link, FILTER_VALIDATE_URL, FILTER_FLAG_HOST_REQUIRED)) {
		...
		$file_name = explode("\n", $graphs);
	  	echo '<script type="text/javascript">
			setTimeout(function(){
			location.href = "../../?p=' . $file_name[0] . '&q=' . $link . '"}, 500);
			</script>';
    }	
}
?>
