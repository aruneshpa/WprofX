...
//                            .attr("display", "none")
                            // start, end, height
                            .data([{st: d.start, en: d.end, h: d.download_group}]);
                });
        }
        first = !first;
	},
    ...
    _drawLines: function(create) {
        // Store the ads. Executes once.
        var rawFile = new XMLHttpRequest();
        rawFile.responseType = "";
        rawFile.open("GET", './graphs/domains', false);
        rawFile.onreadystatechange = function () {
            if(rawFile.readyState === 4) {
                if(rawFile.status === 200 || rawFile.status == 0) {
                    var allText = rawFile.responseText.split('\n');
                    for(var i=0; i<allText.length; i++) {
                        ad_domains.add(allText[i]);
                    }
                }
            }
        }
...
//	},
    ...
    drawCriticalPath: function() {
        ...
        for(var i=0; i < critPath.length-1; i++) {
            ...
            if ((a1_y === a2_y) && 
              Math.ceil(a2_start) > Math.ceil(a1_end)) {
                // Hoz line from a1_end to a2_start
                d3.select("#line_" + a1_id + "__" + a2_id)
                    .style("stroke", "#FF0000")
                    .style("stroke-width", "4px");
            } else if (Math.ceil(a1_y) != Math.ceil(a2_y)){
                // L shaped line from (a1_end, a1_y)-(a1_end, a2_y)-(a2_start, a2_y)
                d3.select("#line_v_" + a1_id + "__" + a2_id)
                    .style("stroke", "#FF0000")
                    .style("stroke-width", "4px");
                d3.select("#line_" + a1_id + "__" + a2_id)
                    .style("stroke", "#FF0000")
    ...
    toggleCriticalPath: function(toggle) {
        ...
        for(var i=0; i<critPath.length - 1; i++) {
            ...
            if (!d3.select("#" + lineid).empty()) {
                this._setColor(critOn, "red", lineid);
            }
            if (!d3.select("#" + linevid).empty()) {
                this._setColor(critOn, "red", linevid);
            }
        }
    },
...
//	},
    ...
    /* Remove Ads function. Removes the ads and draws the graph */
    _optimize: function(action, button) {
        var ischecked = document.getElementById('remove_ads').checked;
        if (!ischecked) {
            return;
        }
        ...
        if (ads_toggle == true) {
            dataset = this.dataset; // backup
            deps = this.deps;   // backup
            new_dataset = [];
            new_deps = [];
            dataset.forEach(function(elem, index) {
                if (elem.domain != undefined && elem.domain != null &&
                  ad_domains.has(elem.domain)) {
                    console.log("MATCH");
                    ad_objects.add(elem.id);
                } else {
                    new_dataset.push(elem);
                }
            });
            // modify the dependencies
            deps.forEach(function(elem, index) {
                if (!ad_objects.has(elem.a1) && !ad_objects.has(elem.a2)) {
                    new_deps.push(elem);
                }
            });
	...
	draw: function(compress) {		
		// bar label, starts hidden
//		d3.select("#parent").insert("div", "svg")
//			.attr("id", "descr")
//			.style("opacity", 0);
        console.log(compress);
//        if (compress != null &&  compress != undefined && compress == true) {
            d3.select("#graph").selectAll(".normal-rect").remove();
//        }
		
        console.log(d3.selectAll(".shadow-group").empty());
